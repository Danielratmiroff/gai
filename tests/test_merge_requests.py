...
def test_get_commits_success(mock_subprocess_run_success, merge_requests_instance):
    ...
    commits = merge_requests_instance.get_commits("main", "feature-branch")
...
def test_get_commits_fetch_failure(mock_subprocess_run_failure, merge_requests_instance):
    commits = merge_requests_instance.get_commits("main", "feature-branch")
    assert commits.startswith(
        "Error fetching commits:"), "Should return error message on fetch failure"


def test_get_commits_log_failure(mock_subprocess_run_success, merge_requests_instance):
    # Mock the fetch command
    mock_fetch = mock_subprocess_run_output("", 0)
